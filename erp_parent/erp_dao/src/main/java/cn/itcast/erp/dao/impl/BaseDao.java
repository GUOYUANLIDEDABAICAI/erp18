...
public class BaseDao<T> extends HibernateDaoSupport implements IBaseDao<T> {
	...
	public void update(T t){
		Map<Long, String> nameMap = cacheManager.getCache("myCache").get(entityClass, Map.class);
		if(null != nameMap){
			nameMap.clear();
		}
	...
	@Override
	public String getObjectName(Long uuid) {
		if(null == uuid){
			return null;
		}
		Map<Long, String> nameMap = cacheManager.getCache("myCache").get(entityClass, Map.class);
		if(null == nameMap){
			...
			cacheManager.getCache("myCache").put(entityClass, nameMap);
		}
		...
		if(null == nameMap.get(uuid)){
			...
			try {
				//通过反射得到getName的方法
				Method getNameMethod = entityClass.getDeclaredMethod("getName");
				//调用getName方法
				Object result = getNameMethod.invoke(t);
				//转成字符串
				name = (String)result;
