...
public class BaseAction<T> {
	...
	public void listByPage(){
		...
		//把部门列表转JSON字符串, DisableCircularReferenceDetect：禁止循环引用 "dep":{"$ref":"$.rows[3].dep"}
		// WriteMapNullValue 如果属性值为空，则输出该属性，且它的值为null
	...
	public void add(){
		...
		WebUtil.write(JSON.toJSONString(rtn));
	}
	...
	public void get(){
		T t = baseBiz.get(id);
		//WithDateFormat如果转换的对象中有日期的属性，且有值的情况下，转指定格式的字符串
	...
	private String mapData(String jsonString, String prefix){
		...
		for(String key : map.keySet()){
			//"t.dep":{"name":"管理员组","tele":"000000","uuid":1} form表单不能显示它，name=t.dep.uuid
			//应该转成这种格式："t.dep.name":"管理员组","t.dep.tele":"000000","t.dep.uuid":1
			if(map.get(key) instanceof Map){
				@SuppressWarnings("unchecked")
				//{"name":"管理员组","tele":"000000","uuid":1}
				Map<String,Object> innerMap = (Map<String,Object>)map.get(key);
				// innerMap.keySet() => name, tele, uuid
				for(String key2 : innerMap.keySet()){
					//key => t.dep.name
					dataMap.put(prefix + "." + key + "." + key2, innerMap.get(key2));
				}
			}else{
