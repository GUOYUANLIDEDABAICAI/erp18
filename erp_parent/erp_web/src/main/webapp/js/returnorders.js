...
$(function() {
	...
	if(oper == "orders"){
		url = 'returnorders_listByPage?t1.type=' + type;
	}
	...
	//审核业务
	if(oper == 'doCheck'){
		url = "returnorders_listByPage?t1.type="+ type +"&t1.state=0";//未审核的订单
		//详情窗口，要加一个审核的按钮
		ordersDlgCfgToolbar.push({
			text:'审核',
			iconCls:'icon-search',
	...
	if(oper == 'doInStore' || oper == 'doOutStore'){
		url = "returnorders_listByPage?t1.type=" + type;
		if(type == 1){
			url += "&t1.state=1";//采购退货单时，显示审核的数据
			document.title="采购退货入库";
		}
		if(type == 2){
			url += "&t1.state=1";//销售退货单时，显示审核的数据
			document.title="销售退货出库";
		}
	...
	//我的订单
	if(oper == "myorders"){
		gridCfg.url = "returnorders_myListByPage?t1.type=" + type;
		var btnName = "采购退货登记";
		if(type == 2){
			btnName = "销售退货登记";
		}
...
function formatState(value){
	//采购退货: 0:未审核 1:已审核, 2:已出库；销售：0:未审核 1:已审核, 2:已出库
	if(type == 1){
		switch (value * 1) {
			case 0: return '未审核';
			case 1: return '已审核';
